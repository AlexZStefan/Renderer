...
Model::Model(const char *filename) : verts_(), faces_(), vts_() {
    ...
    while (!in.eof()) {
        ...
        if (!line.compare(0, 2, "v ")) {       // read 2 characters and check the line starts with "v"
        ...
        else if (!line.compare(0, 3, "vn ")) {       // read 3 characters and check the line starts with "vn"
            iss >> trash;
            iss >> trash;
            Vec3f vn;
            ...
            int itrash, idx, vn_idx, vt_idx;
            iss >> trash;
            while (iss >> idx >> trash >> vt_idx >> trash >> vn_idx) { // read in v_i to idx and discard /vt_i/vn_i
                idx--, vn_idx--; // in wavefront obj all indices start at 1, not zero, we need them to start at zero
                f.push_back(idx);
...
std::vector<int> Model::vnorm(int i) {
    return vnorm_[i];
}

std::vector<int> Model::vtan(int i) {
    return vtan_[i];
}
